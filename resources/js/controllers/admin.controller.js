(function (angular) {
    ...
    function AnalisaController($scope, $http, helperServices, AnalisaService, PeriodeService, SubKriteriaService) {
        ...
        $scope.sumTotal = (item) => {
            ...
            Object.values(item).forEach(element => {
                total += parseFloat(element.hasil)
            });
            return total;
        }
        $scope.simpananalisa = () => {
            AnalisaService.simpananalisa($scope.alternatif).then(x => {
                swal("Information", "Berhasil Menyimpan Data", "success");
    ...
    function LaporanController($scope, $http, helperServices, AnalisaService, PeriodeService, SubKriteriaService) {
        $scope.datas = [];
        $scope.nasabahs = [];
        $scope.kriteria = [];
        $scope.periode = [];
        ...
        $scope.setLaporan = (param) => {
            ...
            $scope.alternatif = [];
            $scope.itemperiodee = {};
            SubKriteriaService.getlaporan(param.idperiode).then(x => {
                if (x.alternatif.length >= 0) {
                    $scope.view = 'matriks';
                    $scope.alternatif = x.alternatif;
                    $scope.alternatif.forEach(element1 => {
                        var total = 0;
                        element1.value.forEach(element => {
                            total += parseFloat(element.hasil)
                        });
                        element1.hasil = total;
                    });
                    console.log($scope.alternatif);
                }
            })
